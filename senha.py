...
# Tenta carregar a imgem do ícone
try:
...
# Função para  copiar a senha gerada para a area de transferencia
def clipper():
    root.clipboard_clear()
    root.clipboard_append(pw_entry.get())

# Função pra limpar os campo de entrada
def clear_entry():
    my_entry.delete(0, ctk.END)
    pw_entry.delete(0, ctk.END)

# Função para validar o comprimento maximo do input
...
ctk.CTkLabel(lf, text="Quantos caracteres (Maximo 32)?", font=("Helvetica", 16)).pack(pady=10)

# Entry para o inout do comprimento da senha com validação
